...
df_melt['var']=df_melt['var'].astype(CategoricalDtype(categories= np.unique(df_melt['var'])[::-1],ordered=True))
...
for i in list_var:
    ...
    y_new=f(x_new)
    mydata = mydata._append(pd.DataFrame({'x': x_new,'y':y_new,'var':np.repeat(i,N)}))

#mydata['var']=mydata['var'].astype("category",categories= np.unique(df_melt['var'])[::-1],ordered=True)
mydata['var']=mydata['var'].astype(CategoricalDtype(categories= np.unique(df_melt['var'])[::-1],ordered=True))
