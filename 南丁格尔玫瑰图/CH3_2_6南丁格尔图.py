...
n_row= mydata.shape[0]  # .shape[0]返回行数, .shape[1]返回列数
angle = np.arange(0,2*np.pi,2*np.pi/n_row)  # np.arange(start,stop,step)返回从start到stop的step间隔的数组
radius = np.array(mydata.Price) # 返回[50, 60, 70, 20,90,110,30]
...
# plt.bar(angle,radius, color='#F9837A',edgecolor="k",width=0.20,alpha=0.9)
# x轴坐标轴标签
