...
for i in df_rowsum.index:
    ...
    mydata = mydata._append(df_grid)

# mydata['type']=mydata['type'].astype("category",categories=categ_table.columns,ordered=True)
mydata['type'] = mydata['type'].astype(CategoricalDtype(categories=categ_table.columns, ordered=True))
...
base_plot = (ggplot() +
             geom_point(aes(x='x', y='y', fill='type'), mydata, shape='o', color="k", size=4,
                        stroke=0.1) +  # The color of the lines between tiles
             geom_text(aes(x='x', y='y', label='label'), data=mydata_label,
                       size=13) +  # The color of the lines between tiles
             scale_fill_brewer(type='qual', palette="Set1") +
             xlab("Name") +
             ylab("Count: 1 row square =" + str(ndeep)) +
             coord_fixed(ratio=1) +
