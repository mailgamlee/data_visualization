...
df['monthf']=df['monthf'].astype(CategoricalDtype(categories=month_label,ordered=True))
...
df['weekdayf']=df['weekdayf'].astype(CategoricalDtype(categories=week_label,ordered=True))
...
df['monthweek']=df.groupby('monthf')['week'].apply(lambda x: x-x.min()+1).reset_index().set_index('level_1').drop('monthf',axis=1)
