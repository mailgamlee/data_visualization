...
#df['Country']=df['Country'].astype(CategoricalDtype(categories= df['Country'],ordered=True))
...
#mydata['Country']=mydata['Country'].astype(CategoricalDtype(categories= df['Country'],ordered=True))
...
# ------------------------------(d) 百分比堆积柱形图-------------------------------------------------------
df = pd.read_csv('Stackedbar_Data.csv')
SumCol_df = df.iloc[:, 1:].sum(axis=1)

# Fix the FutureWarning by creating a copy of the columns before performing the division
df_percentage = df.iloc[:, 1:].div(SumCol_df, axis=0)
df_percentage['Country'] = df['Country']

meanRow_df = df_percentage.iloc[:, :-1].mean(axis=0).sort_values(ascending=True)
Per_df = df_percentage.loc[:, meanRow_df.idxmax()].sort_values(ascending=True)
Sing_df = df_percentage['Country'][Per_df.index]

mydata = pd.melt(df_percentage, id_vars='Country')
