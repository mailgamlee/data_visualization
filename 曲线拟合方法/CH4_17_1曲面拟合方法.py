...
formula = 'z~x+y'
est = ols(formula, data=df).fit()  # ols()用于拟合多项式，data为数据集; fit()用于计算拟合参数
# print(est.summary())

N=30
# np.linspace()用于生成一系列等间隔的数字，参数为起始值，终止值，个数
xmar= np.linspace(min(df.x),max(df.x),N)
ymar= np.linspace(min(df.y),max(df.y),N)
X,Y=np.meshgrid(xmar,ymar)  # np.meshgrid()用于生成网格数据，X,Y为网格的横纵坐标
