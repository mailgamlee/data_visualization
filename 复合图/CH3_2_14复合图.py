...
sns_hist=sns.jointplot(x='x', y='y',  # 设置xy轴，显示columns名称
              ...
              height = 5, ratio = 5,  # 修改：size -> height
              xlim=(0,60),
              marginal_kws=dict(bins=20, color='#7CBC47',  # 将颜色参数移到这里
                                edgecolor='k', alpha=1)  # 直接设置边缘颜色和透明度
              )
...
sns_reg=sns.jointplot(x='x', y='y',  # 设置xy轴，显示columns名称
              ...
              space = 0,  # 设置散点图和布局图的间距
              #height = 5, ratio = 5,  # 散点图与布局图高度比，整型
...
sns_hex =sns.jointplot(x='x', y='y',  # 设置xy轴，显示columns名称
              ...
              marginal_kws=dict(bins=20, color='#D31A8A',  # 将颜色参数移到这里
                                edgecolor='k', alpha=1)   # 直接设置边缘颜色和透明度
              )  # 修改统计注释
...
#g = sns.JointGrid(x="total_bill", y="tip", data=tips, height = 5, ratio = 5)  # 修改：size -> height
g = sns.JointGrid(x="total_bill", y="tip", data=tips)
g = g.plot_joint(plt.scatter,
                color="#00B1B8", s=40, edgecolor="black",linewidth=0.2)
# 修改：shade=True -> fill=True
g = g.plot_marginals(sns.kdeplot, fill=True, color="#00B1B8")
